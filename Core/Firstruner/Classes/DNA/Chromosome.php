...
use DatasDNA\Interfaces\ILuid_Generator;
...
class Chromosome extends CCollection implements IDNA_Component, ILuid_Generator
{
    ...
    {
        parent::__construct(type: gettype(new Brin()));
        $this->LUID = new Luid();
    }

    public function GetLUID(): string
    {
        return (string) $this->LUID;
    }

    public function PoidsTotal(): float
    {
        //  Calcul du poids total de tous les brins
        return array_reduce(
            $this->ToArray(),
            fn($carry, $brin) => $carry + $brin->PoidsTotal(),
            0.0
        );
    }
}
