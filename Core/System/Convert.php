...
class Convert
{
      private const Base64LineBreakPosition = 76;
      private const Base64VectorizationLengthThreshold = 16;
      ...
      // public const DBNull = DBNull::Value();

      private static function convertObjectToObject($objectA, string $objectB) {
      ...
      public static function GetTypeCode(?object $value) : int
      {
            if ($value == null) return TypeCode::Empty;
            //if ($value instanceof IConvertible) return $value->GetTypeCode();
            ...
            switch (gettype($value))
            {
                  ...
                  case "Boolean": return TypeCode::Boolean; // A simple type representing Boolean values of true or false.

                  case "byte" :
                  case "Byte" : return TypeCode::Byte; // An integral type representing unsigned 8-bit integers with values between 0 and 255.

                  case "Char" : return TypeCode::Char; // An integral type representing unsigned 16-bit integers with values between 0 and 65535. The set of possible values for the Char type corresponds to the Unicode character set.

                  case "datetime" :
                  case "DateTime" : return TypeCode::DateTime; // A type representing a date and time value.

                  case "DBNull" : return TypeCode::DBNull; // A database null (column) value.
                  
                  case "float" :
                  case "Decimal" : return TypeCode::Decimal; // A simple type representing values ranging from 1.0 x 10 -28 to approximately 7.9 x 10 28 with 28-29 significant digits.
                  case "Double" : return TypeCode::Double; // A floating point type representing values ranging from approximately 5.0 x 10 -324 to 1.7 x 10 308 with a precision of 15-16 digits.

                  case "Empty" : return TypeCode::Empty; // A null reference.

                  case "int" :
                  case "Int16" : return TypeCode::Int16; // An integral type representing signed 16-bit integers with values between -32768 and 32767.
                  case "Int32" : return TypeCode::Int32; // An integral type representing signed 32-bit integers with values between -2147483648 and 2147483647.
                  case "Int64" : return TypeCode::Int64; // An integral type representing signed 64-bit integers with values between -9223372036854775808 and 9223372036854775807.

                  case "object" :
                  case "Object" : return TypeCode::Object; // A general type representing any reference or value type not explicitly represented by another TypeCode.

                  case "SByte" : return TypeCode::SByte; // An integral type representing signed 8-bit integers with values between -128 and 127.
                  case "Single" : return TypeCode::Single; // A floating point type representing values ranging from approximately 1.5 x 10 -45 to 3.4 x 10 38 with a precision of 7 digits.

                  case "string" :
                  case "String" : return TypeCode::String; // A sealed class type representing Unicode character strings.

                  case "UInt16" : return TypeCode::UInt16; // An integral type representing unsigned 16-bit integers with values between 0 and 65535.
                  case "UInt32" : return TypeCode::UInt32; // An integral type representing unsigned 32-bit integers with values between 0 and 4294967295.
      ...
      public static function IsDBNull(/*[NotNullWhen(true)]*/ $value) : bool
      {
      ...
      // public static function ChangeType(?object $value, TypeCode $typeCode) : ?object
      // {
      //       return ChangeType(value, typeCode, CultureInfo.CurrentCulture);
      // }

      // //[return: NotNullIfNotNull(nameof(value))]
      // public static function ChangeType(?object $value, TypeCode $typeCode, ?IFormatProvider $provider) : ?object
      // {
      //       if ($value == null && ($typeCode == TypeCode::Empty || $typeCode == TypeCode::String || $typeCode == TypeCode::Object))
      //             return null;

      //       if (!($value instanceof IConvertible)) throw new InvalidCastException();
      ...
      //             case TypeCode::DBNull : throw new InvalidCastException(SR.InvalidCast_DBNull);
      //             case TypeCode::Empty : throw new InvalidCastException(SR.InvalidCast_Empty);
      //             default : throw new ArgumentException(SR.Arg_UnknownTypeCode);
      //       }
      // }
